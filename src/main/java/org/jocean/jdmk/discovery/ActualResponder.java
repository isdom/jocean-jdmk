...
import org.jocean.jdmk.comm.AuthInfo;
import org.jocean.jdmk.comm.HttpConnectorAddress;
import org.jocean.jdmk.comm.HttpsConnectorAddress;
import org.jocean.jdmk.comm.RmiConnectorAddress;
...
class ActualResponder extends DiscoveryCommon {
	...
	if (logger.finerOn()) {
        ...
        // Set Logging stuff
        // ------------------------
	localClassName = "org.jocean.jdmk.discovery.ActualResponder" ;
	...
	if ( rcvPDU.getHost() != null ) {
		...
		if (!org.jocean.jdmk.internal.Useful.isLocalHost(rcvPDU.getHost())) {
		    if (logger.finerOn()) {
	...
	if ( setAdp == true ) {
	    ...
	    // ------------------------
	    {
		String val = "org.jocean.jdmk.comm.RmiConnectorServer" ;
		try {
	    ...
	    // ------------------------
	    {
		String val = "org.jocean.jdmk.comm.HttpConnectorServer" ;
		try
		    {
			result = cmf.queryNames(name, Query.match(Query.classattr(),Query.value(val)));
	    ...
	    // ------------------------
	    {
		String val = "org.jocean.jdmk.comm.HttpsConnectorServer" ;
		try
		    {
			result = cmf.queryNames(name, Query.match(Query.classattr(),Query.value(val)));
	    ...
	    // ------------------------
	    {
		String val = "org.jocean.jdmk.comm.HtmlAdaptorServer" ;
		try {
		    result = cmf.queryNames(name, Query.match(Query.classattr(),Query.value(val)));
	    ...
	    // ------------------------
	    {
		String val = "org.jocean.jdmk.comm.SnmpAdaptorServer" ;
		try {
		    result = cmf.queryNames(name, Query.match(Query.classattr(),Query.value(val)));
